...
import { QuestionWithOptions } from 'src/app/supabase.service';
...
export class QuestionComponent implements OnInit {
  ...
  isTagChecked(tagID: number | null): boolean {
    ...
    if (tagID == -1 && this.question.questionoptions) {
      ...
      return this.question.questionoptions.every(to => !this.tagIsSelected(to.tagid))
    }
  ...
  radioChanged(event: MatRadioChange): void {
    this.question?.questionoptions.forEach(to => {
      if (to.tagid == null) {
        return
      }
